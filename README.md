...
- **Recommended files** in Quick Open (`Ctrl+P`): the empty-query list is your most-used files ranked by frecency (frequency + recency) across *every* project you've opened, persisted between sessions. Typing filters the workspace as usual, but a file you open often is nudged up within its match group; out-of-project favorites that match appear in a small trailing group, marked with a `↗` badge and a `~`-abbreviated path. Remove an entry you don't want recommended with the row's `✕` or `Shift+Delete` (it's dropped from the ranking, not deleted from disk); entries whose file no longer exists are pruned automatically. `Shift+Delete` needs a [Kitty keyboard protocol](#terminal-support) terminal to disambiguate from a plain `Delete`; on other terminals, use the row's `✕` instead.
